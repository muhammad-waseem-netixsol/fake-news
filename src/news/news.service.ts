...
export class NewsService {
  constructor(@InjectModel(News.name) private newsModel: mongoose.Model<News> ,@InjectModel(Newsdata.name) private newsDataSchema: mongoose.Model<Newsdata>,  @InjectModel(Like.name) private likeModel: mongoose.Model<Like>){}
  ...
  async paginatedNews(page: number, pageSize: number) {
    try {
      const result = await this.newsModel
        .find()
        .populate({ path: 'reference', model: 'Newsdata', options: { strictPopulate: false } })
...
async getAnalytics() {
  ...
  const intervals = [];
  for (let currentDate = startDate; currentDate <= endDate; currentDate.setDate(currentDate.getDate() + intervalDays)) {
  ...
  const results = await Promise.all(
    pipelines.map(pipeline => this.newsModel.aggregate([pipeline, { $group: { _id: '$language', count: { $sum: 1 } } }]))
  );

  const dates = intervals.map(formatDate);
  const englishCounts = results.flatMap(result => result.find(item => item._id === 'english')?.count || 0);
  const spanishCounts = results.flatMap(result => result.find(item => item._id === 'spanish')?.count || 0);
  const frenchCounts = results.flatMap(result => result.find(item => item._id === 'french')?.count || 0);
  const turkishCounts = results.flatMap(result => result.find(item => item._id === 'french')?.count || 0);
...
async updateTable1WithReferences(): Promise<void> {
  try {
    ...
    // const test = await this.newsModel.findByIdAndUpdate('65ccfedde956e832af08648a', 
    //   { $set: { language: 'news' } },
    //   { new: true }
    // )
    console.log({test})
    ...
    console.error('An error occurred while updating Table 1 with modified fields:', error);
